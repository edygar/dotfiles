...
local exepath = vim.defaulttable(function(cmd)
  ...
  if git == "" then return false end

  cmd = vim.fs.joinpath(vim.fs.joinpath(vim.fs.dirname(vim.fs.dirname(git)), "usr/bin"), cmd)
  return vim.fn.executable(cmd) == 1 and cmd or false
end)

---@return string
local function preview_get_filler() return vim.opt_local.fillchars:get().diff or "-" end
...
local function preview_decorate(win)
  ...
  vim.uv.fs_stat(
    ...
    vim.schedule_wrap(function(_, stat)
      if not stat or preview_buf_get_path(buf) ~= path then return end
...
local function preview_win_set_lines(win, lines, path)
  ...
  local buf = vim.fn.winbufnr(win)
  if preview_buf_get_path(buf) ~= path then return end
...
local function preview_set_lines(win)
  ...
  if not stat then
    ...
    preview_win_set_lines(win, preview_msg("Invalid path", win_height, win_width), path)
    return
  end
  ...
  if stat.type == "directory" then
    ...
    if not ls_cmd then
      preview_win_set_lines(win, preview_msg("`ls` is required to previous directories", win_height, win_width), path)
      return
    end
  ...
  local function preview_file()
    if vim.fn.winbufnr(win) ~= buf then return end

    vim.b[buf]._dropbar_preview_syntax = bufname
    preview_win_set_lines(
      win,
      vim.iter(io.lines(path)):take(win_height):map(function(line) return (line:gsub("\x0d$", "")) end):totable(),
      path
    )
  end
  ...
  vim.system(
    ...
    vim.schedule_wrap(function(obj)
      ...
      vim.b[buf]._dropbar_preview_msg_shown = bufname
      preview_win_set_lines(win, preview_msg("Binary file", win_height, win_width), path)
    end)
  )
end
...
local function preview(sym)
  if not configs.eval(configs.opts.sources.path.preview, sym.data.path) then return end
  ...
  if not preview_buf or not vim.api.nvim_buf_is_valid(preview_buf) then
    ...
    vim.api.nvim_win_call(sym.win, function() vim.api.nvim_set_current_buf(preview_buf) end)
  end
  ...
  local preview_bufnewname = ("dropbar_preview_%d://%s"):format(preview_buf, path)
...
local function preview_restore_view(sym)
  if not sym.win or not sym.entry or not sym.entry.menu then return end

  local source_buf = sym.entry.menu:root().prev_buf
  if source_buf and vim.api.nvim_buf_is_valid(source_buf) then vim.api.nvim_win_set_buf(sym.win, source_buf) end

  if sym.view then vim.api.nvim_win_call(sym.win, function() vim.fn.winrestview(sym.view) end) end
end
...
local function convert(path, buf, win, label)
  ...
  return bar.dropbar_symbol_t:new(setmetatable({
    buf = buf,
    win = win,
    name = label or vim.fs.basename(path),
    icon = icon,
    name_hl = name_hl,
    icon_hl = icon_hl,
    data = { path = path },
    ...
    __index = function(self, k)
      if k == "children" then
        ...
        if count > 0 then table.insert(self.children, 1, convert(vim.fs.dirname(path), buf, win, "..")) end
      ...
      if k == "siblings" or k == "sibling_idx" then
        ...
        for idx, name in vim.iter(vim.fs.dir(parent_dir)):enumerate() do
          if path_opts.filter(name) then
            ...
            count = count + 1
            if name == self.name then self.sibling_idx = idx end
          end
        end

        if count > 0 then table.insert(self.siblings, 1, convert(vim.fs.dirname(parent_dir), buf, win, "..")) end
...
local function get_symbols(buf, win, _)
  ...
  table.insert(symbols, 1, convert(current_path, buf, win, fs_normalize(vim.fn.fnamemodify(current_path, ":."))))

  if vim.bo[buf].mod then symbols[#symbols] = path_opts.modified(symbols[#symbols]) end

...
function __fzf_select_custom__() {
	local cmd dir
...
function fzf-file-widget-custom() {
	local selected=""
...
# bindkey -m emacs-standard -x "\C-t" "fzf-file-widget-custom"
# bindkey -m emacs-standard -x "\et" "fzf-file-widget-custom no-ignore"
# bindkey -m vi-command -x "\C-t" fzf-file-widget-custom
# bindkey -m vi-command -x "\et" fzf-file-widget-custom no-ignore
# bindkey -m vi-insert -x "\C-t" fzf-file-widget-custom
# bindkey -m vi-insert -x "\et" fzf-file-widget-custom no-ignore

# }}}
# Dirs {{{

# Helper that defines actions for keys in directory-like maps
function __fzf_cd_action_key__() {
	out="$1"
	key=$(head -1 <<<"$out")
...
if [[ -o vi ]]; then
	...
	bindkey "\ec" "\C-x\C-addi$(__fzf_cd_custom__)\C-x\C-e\C-x\C-r\C-m"
	# bind -m vi-command '"\ec": "i\ec"'
	# shellcheck disable=SC2016
	bindkey "\ed" "\C-x\C-addi$(__fzf_cd_custom__ no-ignore)\C-x\C-e\C-x\C-r\C-m"
	# bind -m vi-command '"\ed": "i\ed"'
else
	# shellcheck disable=SC2016
	bindkey "\ec" " \C-e\C-u$(__fzf_cd_custom__)\e\C-e\er\C-m"
	# shellcheck disable=SC2016
	bindkey "\ed" " \C-e\C-u$(__fzf_cd_custom__ no-ignore)\e\C-e\er\C-m"
fi

# Alt-h map to cd from home dir
if [[ -o vi ]]; then
	# shellcheck disable=SC2016
	bindkey "\eh" "\C-x\C-addi$(__fzf_cd_custom__ no-ignore ~)\C-x\C-e\C-x\C-r\C-m"
	# bind -m vi-command '"\eh": "i\eh"'
else
	# shellcheck disable=SC2016
	bindkey "\eh" " \C-e\C-u$(__fzf_cd_custom__ no-ignore ~)\e\C-e\er\C-m"
fi

# Alt-p mapping to cd to selected parent directory (sister to Alt-c)
function __fzf_cd_parent__() {
	local dirs=()
	get_parent_dirs() {
		if [[ -d "${1}" ]]; then
			dirs+=("$1")
...
if [[ -o vi ]]; then
	# shellcheck disable=SC2016
	bindkey "\ep" "\C-x\C-addi$(__fzf_cd_parent__)\C-x\C-e\C-x\C-r\C-m"
	bind -m vi-command '"\ep": "i\ep"'
else
	# shellcheck disable=SC2016
	bindkey "\ep" " \C-e\C-u$(__fzf_cd_parent__)\e\C-e\er\C-m"
fi
...
if [[ -o vi ]]; then
	# shellcheck disable=SC2016
	bindkey "\ez" "\C-x\C-addi$(z)\C-x\C-e\C-x\C-r\C-m"
	# shellcheck disable=SC2016
	bindkey -m vi-command "\ez" "ddi$(z)\C-x\C-e\C-x\C-r\C-m"
else
	# shellcheck disable=SC2016
	bindkey "\ez" " \C-e\C-u$(z)\e\C-e\er\C-m"
fi
